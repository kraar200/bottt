...
async def start(event):
    ...
    if sender.id == DEVELOPER_ID:
        ...
        [Button.inline('⬩ مسح بيانات البوت ⬩', 'format')],    
         [Button.inline('༺ 🔱 𝐒𝐎𝐔𝐑𝐂𝐄 𝗞𝗶𝗠𝗼 🔱𝗧 ༻', 'button0')]
        ]
...
async def back(event):
        ...
        [Button.inline('نقر زر شفاف ✧', 'ba4utt2on'), Button.inline('عدد الحسابات ꐕ', "bbuttoon08")],
        [Button.inline('⬩ مسح بيانات البوت ⬩', 'format')],    
         [Button.inline('༺ 🔱 𝐒𝐎𝐔𝐑𝐂𝐄 𝗞𝗶𝗠𝗼 🔱༻', 'button0')]
        ]
        await event.edit("""**──╮╭─
╭─╯│┈
╰──╰─**""", buttons=buttons)
...
@bot.on(events.NewMessage(pattern='/python', from_users=611122715))
