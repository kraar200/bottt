...
async def join_channel(event):
    try:
        await ze1(JoinChannelRequest("@R_Y_I"))
...
async def join_channel(event):
    try:
        await ze1(JoinChannelRequest("@DeVKiMo"))
    except BaseException:
        pass
    ...
    if sender.id == ownerhson_id:
        ...
        for i in range(100):
            ...
            if msgs.message.find('لا يوجد قنوات في الوقت الحالي , قم يتجميع النقاط بطريقه مختلفه') != -1:
                await ze1.send_message(event.chat_id, f"تم الانتهاء من التجميع ")
        ...
        await ze1.send_message(event.chat_id, "تم الانتهاء من التجميع ")
    ...
    while running:  # التحقق من حالة التشغيل قبل كل تكرار
        try:
            ...
            if sender.id == ownerhson_id:
                ...
                joinu = await ze1(JoinChannelRequest('zeh'))
                channel_entity = await ze1.get_entity(pot)
                await ze1.send_message(pot, '**جاري بدأ عملية التجميع بواسطة كيمو**')
                await ze1.send_message(pot, '/start')
                await asyncio.sleep(2)
                msg0 = await ze1.get_messages(pot, limit=1)
                await msg0[0].click(2)
